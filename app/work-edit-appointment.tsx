...
import {
  deleteWorkAppointment,
  getWorkAppointment,
  updateWorkAppointment,
...
export default function WorkEditAppointmentScreen() {
  ...
  useEffect(() => {
    if (!id) return;
    getWorkAppointment(id)
      .then((value) => {
        if (!value || value.source === 'website') {
      ...
      .catch(() => router.replace('/work-calendar'));
  }, [id, router]);
...
const styles = StyleSheet.create({
  ...
  closeText: { color: colors.work, fontSize: 34 },
  screenTitle: { color: colors.textPrimary, fontSize: 28, fontWeight: '700' },
  dateCard: { backgroundColor: colors.workSoft, borderRadius: 22, marginTop: 12, padding: 18 },
  ...
  chevron: { color: colors.work, fontSize: 22 },
  deleteButton: { alignItems: 'center', marginTop: 28, padding: 14 },
  deleteText: { color: '#B23636', fontSize: 19, fontWeight: '700' },
  saveButton: { alignItems: 'center', backgroundColor: colors.work, borderRadius: 22, height: 68, justifyContent: 'center', marginTop: 'auto' },
