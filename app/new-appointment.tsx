  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        ...
        <Pressable disabled={disabled} onPress={save} style={[styles.saveButton, disabled && styles.disabled]}><Text style={styles.saveText}>{saving ? 'Lagrer...' : 'Lagre avtale'}</Text></Pressable>
      </View>
...
const styles = StyleSheet.create({
  ...
  closeText: { color: colors.private, fontSize: 34 }, screenTitle: { color: colors.textPrimary, fontSize: 28, fontWeight: '700' },
  dateCard: { backgroundColor: colors.privateSoft, borderRadius: 22, marginTop: 12, padding: 18 }, dateLabel: { color: colors.textSecondary, fontSize: 16, fontWeight: '600' },
  ...
  timeText: { color: colors.textPrimary, flex: 1, fontSize: 28, fontWeight: '600' }, chevron: { color: colors.private, fontSize: 22 },
  ...
  saveButton: { alignItems: 'center', backgroundColor: '#2FB98E', borderRadius: 22, height: 68, justifyContent: 'center', marginTop: 'auto' }, disabled: { opacity: 0.45 }, saveText: { color: colors.white, fontSize: 24, fontWeight: '700' },
});
