...
import { disconnectWorkCalendar, getWorkSyncConnection } from '@/src/services/workCalendarSync';
...
export default function SettingsScreen() {
  ...
  useEffect(() => {
    Promise.all([getNotificationSettings(), readAppPreferences(), getWorkSyncConnection()]).then(([notification, preferences, connection]) => {
  ...
  function confirmDisconnect() {
    Alert.alert('Koble fra PC?', 'Importerte Opptatt-perioder fjernes. Lokale avtaler beholdes.', [
      ...
      { text: 'Koble fra', style: 'destructive', onPress: async () => {
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.card}>
          ...
          <View style={styles.options}>{REMINDER_OPTIONS.map((option) => {
        ...
        {pcConnected && <View style={styles.card}>
          <Text style={styles.sectionTitle}>PC-synk</Text>
          <Text style={styles.help}>Telefonen er koblet til en lokal PC.</Text>
          <Pressable onPress={confirmDisconnect} style={styles.dangerButton}><Text style={styles.dangerText}>Koble fra PC</Text></Pressable>
        </View>}
        <Text style={styles.note}>En Ny Dag låses igjen etter 2 minutter i bakgrunnen.</Text>
      </ScrollView>
...
function SettingSwitch({ title, description, value, onChange }: { title: string; description: string; value: boolean; onChange: (value: boolean) => void }) {
  return <View style={styles.switchRow}><View style={styles.switchText}><Text style={styles.sectionTitle}>{title}</Text><Text style={styles.help}>{description}</Text></View><Switch value={value} onValueChange={onChange} /></View>;
}

const styles = StyleSheet.create({
  safeArea: { backgroundColor: colors.background, flex: 1, padding: 18 }, header: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }, headerButton: { alignItems: 'center', height: 44, justifyContent: 'center', width: 44 }, backText: { color: colors.private, fontSize: 40, lineHeight: 42 }, title: { color: colors.textPrimary, fontSize: 25, fontWeight: '700' },
  content: { gap: 14, paddingBottom: 20, paddingTop: 20 }, card: { backgroundColor: colors.surface, borderRadius: 24, padding: 20 }, sectionTitle: { color: colors.textPrimary, fontSize: 20, fontWeight: '700' }, help: { color: colors.textSecondary, fontSize: 15, marginTop: 4 }, note: { color: colors.textSecondary, fontSize: 14, textAlign: 'center' },
  ...
  switchRow: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }, switchText: { flex: 1, paddingRight: 14 }, secondaryButton: { alignItems: 'center', borderColor: colors.private, borderRadius: 17, borderWidth: 1.5, height: 50, justifyContent: 'center', marginTop: 18 }, secondaryText: { color: colors.private, fontSize: 17, fontWeight: '700' }, dangerButton: { alignItems: 'center', marginTop: 16, padding: 12 }, dangerText: { color: '#B23636', fontSize: 17, fontWeight: '700' },
