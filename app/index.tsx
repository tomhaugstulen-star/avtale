    ...
    <ImageBackground source={background} style={styles.background} resizeMode="cover">
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.header}>
          ...
          <Text adjustsFontSizeToFit numberOfLines={1} style={styles.title}>{greeting}</Text>
          <Text numberOfLines={2} style={styles.subtitle}>Hvilken kalender vil du se i dag?</Text>
        </View>
        <View style={styles.cards}>
          <CalendarChoiceCard title="Privat" accent={colors.private} softBackground={colors.privateSoft} icon={privateIcon} onPress={() => router.push('/calendar')} />
          <CalendarChoiceCard title="En Ny Dag" accent={colors.work} softBackground={colors.workSoft} icon={workIcon} onPress={() => router.push('/work-lock')} />
        </View>
...
const styles = StyleSheet.create({
  ...
  settingsButton: { alignSelf: 'flex-end', padding: 12 },
  settingsText: { color: colors.textPrimary, fontSize: 16, fontWeight: '700' },
  header: { alignItems: 'center', marginTop: 2 },
  sun: { color: '#F1B542', fontSize: 52, lineHeight: 56, marginBottom: 12 },
  title: { color: colors.textPrimary, fontSize: 42, fontWeight: '600', letterSpacing: -1, textAlign: 'center' },
  subtitle: { color: colors.textSecondary, fontSize: 20, lineHeight: 27, marginTop: 10, textAlign: 'center' },
  cards: { flexDirection: 'row', gap: 14, marginTop: 36 },
  privacy: { color: '#4B5563', fontSize: 14, fontWeight: '600', marginTop: 'auto', paddingBottom: 18, textAlign: 'center' },
});
